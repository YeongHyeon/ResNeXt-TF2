...
class CNN(object):
    ...
    def step(self, x, y, iteration=0, train=False):
        ...
        if(train):
            ...
            with self.summary_writer.as_default():
                tf.summary.scalar('ResNeXt/loss', loss, step=iteration)
                tf.summary.scalar('ResNeXt/accuracy', accuracy, step=iteration)
    ...
    def residual_X(self, input, ksize, inchannel, outchannel, name="", verbose=False):

        convtmp_1 = self.customlayers.conv2d(input, \
            self.customlayers.get_weight(vshape=[1, 1, inchannel, inchannel//2], name="%s_1" %(name)), \
            stride_size=1, padding='SAME')
        ...
        convtmp_2 = self.customlayers.conv2d(convtmp_1act, \
            self.customlayers.get_weight(vshape=[ksize, ksize, inchannel//2, inchannel], name="%s_2" %(name)), \
            stride_size=1, padding='SAME')
        ...
        convtmp_3 = self.customlayers.conv2d(convtmp_2act, \
            self.customlayers.get_weight(vshape=[ksize, ksize, inchannel, outchannel], name="%s_3" %(name)), \
            stride_size=1, padding='SAME')
